...
public final class LoadBalancerUriTools {
	...
	public static URI reconstructURI(ServiceInstance serviceInstance, URI original) {
		if (serviceInstance == null) {
			...
			String tlsMode = serviceInstance.getMetadata().get("mseTlsEnabled");
			if (tlsMode != null && !tlsMode.isEmpty()) {
				if ("http".equals(original.getScheme())) {
					String uriSt = original.toString();
					String uriStNew = uriSt.replaceFirst("http", "https");
					try {
						original = new URI(uriStNew);
