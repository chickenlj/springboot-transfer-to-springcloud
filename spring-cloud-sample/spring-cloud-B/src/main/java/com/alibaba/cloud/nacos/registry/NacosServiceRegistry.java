...
public class NacosServiceRegistry implements ServiceRegistry<Registration> {
	...
	public void register(Registration registration) {
		...
		if ("permissive".equalsIgnoreCase(mseTlsMode)) {
			log.info("[Security] Register to nacos, permissive mode");
			instance.addMetadata("mseTlsEnabled", "permissive");
		...
		// 启动安全，严格模式单端口
//		if(mseSecurityEnabled && strict){
//			port = mse.getAppPort();
//			if(port != instance.getPort() ) {
//				log.warn("提醒端口被切换了，建议是用户配置为和agent一致");
...
//		}
	...
	public void deregister(Registration registration) {
		...
		// 如果端口不一致，修改端口为tls端口
		if (tlsPort != registration.getPort()) {
			log.info(
					"[Security] Deregister from nacos, port of registration change from {} to {}.",
					registration.getPort(), tlsPort);
//			NacosRegistration nacosRegistration = (NacosRegistration)registration;
//			NacosDiscoveryProperties properties = nacosRegistration.getNacosDiscoveryProperties();
			nacosDiscoveryProperties.setPort(tlsPort);
		}
