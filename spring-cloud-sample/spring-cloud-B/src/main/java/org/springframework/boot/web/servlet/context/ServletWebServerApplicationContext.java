...
import org.springframework.boot.web.servlet.context.cert.XdsSslStoreProvider;
...
public class ServletWebServerApplicationContext extends GenericWebApplicationContext
	...
	protected void onRefresh() {
		...
		try {
			...
			if ("permissive".equalsIgnoreCase(tlsMode)) {
				...
				createAnotherWebServer(tlsPort);
			}
		}
		catch (Throwable ex) {
	...
	private void createAnotherWebServer(int tlsPort){
		ServletWebServerFactory factory = getWebServerFactory();

		if (factory instanceof TomcatServletWebServerFactory) {
			StartupStep createWebServer = this.getApplicationStartup().start("spring.boot.webserver.create");
			TomcatServletWebServerFactory existingWebServerFactory = (TomcatServletWebServerFactory) factory;
			existingWebServerFactory.setPort(tlsPort);
			existingWebServerFactory.setSsl(getSsl());
			existingWebServerFactory.setSslStoreProvider(new XdsSslStoreProvider());
			WebServer webServer = existingWebServerFactory.getWebServer(getSelfInitializer());

			getBeanFactory().registerSingleton("webServerGracefulShutdown-agent",
					new WebServerGracefulShutdownLifecycle(webServer));
			getBeanFactory().registerSingleton("webServerStartStop-agent",
	...
	private void createWebServer() {
		...
		if (webServer == null && servletContext == null) {
			...
			if ("strict".equalsIgnoreCase(tlsMode)) {
				...
				int tlsPort = Integer.parseInt(rawTlsPort);
				webServerFactory.setPort(tlsPort);
				webServerFactory.setSsl(getSsl());
				webServerFactory.setSslStoreProvider(new XdsSslStoreProvider());
			}
